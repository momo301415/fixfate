...
class K84Controller extends GetxController {
  ...
  /// 繪製圖表--日
  List<FlSpot> buildDaySpots(List<StepDataData> data) {
  ...
  /// 繪製圖表--週
  List<FlSpot> buildWeeklySpots(List<StepDataData> data, DateTime startDate) {
    List<FlSpot> spots = [];
    for (int i = 0; i < 7; i++) {
      ...
      if (dayData.isNotEmpty) {
        final avg = dayData.map((e) => e.distance).reduce((a, b) => a + b) /
            dayData.length;
  ...
  /// 繪製圖表--月
  List<FlSpot> buildMonthlySpots(List<StepDataData> data, DateTime startDate) {
    List<FlSpot> spots = [];
    final daysInMonth =
