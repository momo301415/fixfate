...
import 'package:pulsedevice/presentation/k84_page/mdoel/k84_model.dart';
import 'package:pulsedevice/presentation/k84_page/mdoel/list_item_model.dart';
...
class K84Controller extends GetxController with WidgetsBindingObserver {
  final gc = Get.find<GlobalController>();
  final k84ModelObj = K84Model().obs;
  ...
  final distanceVal = ''.obs;
  final loadDataTime = ''.obs;
  final stepData = <StepDataData>[].obs;
  final stepApiData = <DistanceData>[].obs;
  final _maxY = 0.obs;
  ...
  Future<void> loadDataByDate(DateTime date, {bool isLoading = true}) async {
    ...
    try {
      ...
      if (res.isNotEmpty && res["message"] == "SUCCESS") {
        ...
        List<DistanceData> parsed = [];
        if (rateData is List) {
          parsed = rateData.map((e) => DistanceData.fromJson(e)).toList();
        }
        ...
        final stepList = parsed.map((e) => int.parse(e.distance)).toList();
        distanceVal.value =
        ...
        final list = parsed.map((m) {
          ...
          return ListHistoryItemModel(
            unit: Rx('lbl193'.tr),
            value: Rx(m.distance.toString()),
            time: Rx(date),
          );
        }).toList();
        k84ModelObj.value.listItemList2.value = list;
      }
    } catch (e) {
  ...
  //   loadDataTime.value =
  //       DateTimeUtils.getTimeDifferenceString(lastData.startTimeStamp);
  //   final stepList = res.map((e) => e.distance).toList();
  //   distanceVal.value = stepList.fold<int>(0, (sum, e) => sum + e).toString();
    ...
    if (index == 0) {
      ...
      if (stepApiData.isEmpty) {
        ...
        data = stepApiData.map((e) {
          ...
          return FlSpot(diffMinutes, double.parse(e.distance));
        }).toList();
  ...
  //             DateTime.fromMillisecondsSinceEpoch(e.startTimeStamp * 1000);
  //         final diffMinutes = current.difference(base).inMinutes.toDouble();
  //         return FlSpot(diffMinutes, e.distance.toDouble());
  //       }).toList();
  ...
  //         if (diffDays >= 0 && diffDays < 7) {
  //           dayData.putIfAbsent(diffDays, () => []).add(e.distance);
  //         }
  //       }
  ...
  void clearData() {
    ...
    k84ModelObj.value.listItemList2.value.clear(); // 歷史紀錄
    k84ModelObj.value.listItemList2.refresh();
  }

  /// 記錄頁面訪問事件
  void _logPageViewEvent() {
    FirebaseAnalyticsService.instance.logViewDistancePage(
      distanceValue: distanceVal.value.isNotEmpty ? distanceVal.value : null,
      parameters: {
