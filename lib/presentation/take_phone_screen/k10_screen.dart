...
class ScanFoodScreen extends GetWidget<CameraScreenController> {
        ...
        children: [
          ...
          Obx(() {
            ...
            if (controller.isAnalyzing.value) {
              return Positioned.fill(child: AnalyzingOverlayWidget(controller: controller));
            }
          ...
          Obx(() {
            if (controller.analysisResult.value != null &&
                !controller.isAnalyzing.value) {
              return _buildAnalysisResultPreview();
            } 
            return SizedBox.shrink();
          }),
        ],
  ...
  // 分析状态覆盖层
  // 分析结果快速预览
  Widget _buildAnalysisResultPreview() {
          ...
          children: [
            Row(
              children: [
                ...
                GestureDetector(
                  onTap: () {
                    controller.analysisResult.value = null;
                  },
                  child: Icon(
                    Icons.close,
                    size: 20,
            ...
            Row(
              ...
              children: [
                _buildNutrientInfo(
                    '总卡路里', '${result.totalCalories.toStringAsFixed(0)} kcal'),
                _buildNutrientInfo('食物种类', '${result.foods.length}种'),
              ],
            ),
            SizedBox(height: 16.h),
            Row(
              children: [
                Expanded(
                  child: ElevatedButton(
                    onPressed: () {
                      Get.to(() => NutritionAnalysisResultScreen(
