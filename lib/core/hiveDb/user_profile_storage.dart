...
class UserProfileStorage {
  ...
  static Future<void> saveFitDeviceForCurrentUser(
      String userId, PPDeviceModel device) async {
    UserProfile? user = _box.get(userId);

    final newDevice = DeviceProfile(
      macAddress: device.deviceMac ?? "",
      deviceIdentifier: device.deviceMac ?? "",
      name: device.deviceName ?? "",
      rssiValue: device.rssi ?? 0,
      firmwareVersion: 0,
      deviceSize: 0,
      deviceColor: 0,
      imageIndex: 0,
    ...
    if (user == null) {
      ...
      // 檢查是否已存在相同裝置（用 macAddress）
      final index =
          user.devices?.indexWhere((d) => d.macAddress == device.deviceMac);
      if (index != -1) {
        user.devices?[index!] = newDevice;
      } else {
        user.devices?.add(newDevice);
